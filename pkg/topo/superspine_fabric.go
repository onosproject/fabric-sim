...
func createRackPairFabric(rackID int, fabric *SuperSpineFabric, builder *Builder, topology *Topology) {
	...
	builder.minPort[leaf11] = builder.nextPort[leaf11]
	builder.minPort[leaf12] = builder.nextPort[leaf12]
	builder.minPort[leaf21] = builder.nextPort[leaf21]
	builder.minPort[leaf22] = builder.nextPort[leaf22]

	// Finally, create hosts with dual interfaces to the paired leaves
	createRackHosts(rackID, leaf11, leaf12, 10, false, 0, builder, topology, coord(2*rackID-1, 4, leafGap, -3*leafGap/2), hostsPerRow)
	createRackHosts(rackID+1, leaf21, leaf22, 10, false, 0, builder, topology, coord(2*rackID+0, 4, leafGap, -leafGap/2), hostsPerRow)
}
