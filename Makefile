...
recipes:	# HELP generate topology files from the topology recipes
	@for topo in "plain_small" "plain" "plain_large" "access" "access2" "superspine"; do go run cmd/fabric-sim-topo/fabric-sim-topo.go gen topo --recipe topologies/$${topo}_recipe.yaml --output topologies/$${topo}.yaml; done

netcfg:	# HELP generate netcfg files from the topology files
	@for topo in "trivial" "plain_small" "plain" "plain_large" "access" "access2" "superspine"; do go run cmd/fabric-sim-topo/fabric-sim-topo.go gen netcfg --topology topologies/$${topo}.yaml --output topologies/$${topo}_netcfg.json; done

robot:	# HELP generate Robot test topology files from the topology files
	@for topo in "trivial" "plain_small" "plain" "plain_large" "access" "access2" "superspine"; do go run cmd/fabric-sim-topo/fabric-sim-topo.go gen robot --topology topologies/$${topo}.yaml --output topologies/$${topo}_robot.yaml; done
